...
const handleEndShed = async (device_id: string, startTime?: Date, clientEventId?: string): Promise<EventSchemaType> => {
    ...
    console.log(`[DEBUG] EndShed handler using event_id: ${eventId} ${clientEventId ? '(client-provided)' : '(generated for backward compatibility)'}`);
