...
const handleGridEmergency = async (device_id: string, startTime?: Date, clientEventId?: string): Promise<EventSchemaType> => {
    ...
    // Create binary payload - only need event type and start time for GRID_EMERGENCY
    const buffer = new ArrayBuffer(5) // 1 byte for event type + 4 bytes for time
    const view = new DataView(buffer)
    view.setUint8(0, EventMap.GRID_EMERGENCY) // Event type
    ...
    console.log(`[DEBUG] GridEmergency handler using event_id: ${eventId} ${clientEventId ? '(client-provided)' : '(generated for backward compatibility)'}`);
